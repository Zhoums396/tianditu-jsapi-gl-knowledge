...
适用于 **LineString** 和 **MultiLineString** 几何类型的 GeoJSON 数据。
        ...
        map.on("load", function() {
            fetch("${DATA_URL}")
                ...
                .then(function(geojsonData) {
                    ...
                    map.on("mouseenter", "lines-layer", function() { map.getCanvas().style.cursor = "pointer"; });
                    map.on("mouseleave", "lines-layer", function() { map.getCanvas().style.cursor = ""; });
                })
                .catch(function(err) {
...
| line-width | 线宽度 | 3 |
| line-opacity | 透明度 | 0.8 |
| line-dasharray | 虚线样式 | [2, 2] |
| line-cap | 端点样式 | "round" |
| line-join | 连接样式 | "round" |
